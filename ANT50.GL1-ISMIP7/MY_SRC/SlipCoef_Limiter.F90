...
FUNCTION Calcul_Slc_Lim(model, nodenumber, VarIn) RESULT(VarOut)
   USE DefUtils
   IMPLICIT NONE
   TYPE(Model_t) :: model
   INTEGER :: nodenumber
   ...
   ! get needed variable
   hth  = ListGetConstReal( Material, 'SSA Friction Threshold Height',UnFoundFatal=.TRUE.)
   slc_min  = ListGetConstReal( Material, 'SSA Friction CR Coeff Min' ,UnFoundFatal=.TRUE.)
   slc_max  = ListGetConstReal( Material, 'SSA Friction CR Coeff Max' ,UnFoundFatal=.TRUE.)

   ! non modified slip coneficient (slc0)
   slc0 = VarIn(1)
   slc0 = max(slc_min,slc0)
   slc0 = min(slc0, slc_max)
...
FUNCTION Calcul_Slc_reduc (model, nodenumber, VarIn) RESULT(VarOut)
   USE DefUtils
   IMPLICIT NONE
   TYPE(Model_t) :: model
   INTEGER :: nodenumber
   ...
   IF ( lda .LE. 1.0 ) slc0 = slc0 * factor

   VarOut = slc0

END FUNCTION Calcul_Slc_reduc
